...
cdef extern from "mliap_data.h" namespace "LAMMPS_NS":
    cdef cppclass MLIAPData:
        ...
        # new data to be passed to python
        double ** positions
        double ** velocities
        double * masses
...
cdef class MLIAPDataPy:
    ...
    def positions(self):
        if self.data.positions is NULL:
            return None
        return np.asarray(<double[:self.ntotal, :3]> &self.data.positions[0][0])

    @property
    def velocities(self):
        if self.data.velocities is NULL:
            return None
        return np.asarray(<double[:self.ntotal, :3]> &self.data.velocities[0][0])

    @property
    def masses(self):
        if self.data.masses is NULL:
            return None
        return np.asarray(<double[:self.ntotal]> &self.data.masses[0])
