...
MLIAPData::MLIAPData(LAMMPS *lmp, int gradgradflag_in, int *map_in, class MLIAPModel *model_in,
    ...
    graddesc(nullptr), model(nullptr), descriptor(nullptr), list(nullptr), positions(nullptr),
    velocities(nullptr), masses(nullptr)  // added positions and velocities and masses
{
...
MLIAPData::~MLIAPData()
{
  ...
  memory->destroy(positions);
  memory->destroy(velocities);
