...
enum {
  ...
  POSITIONS_MASK   = 0x00020000,
  VELOCITIES_MASK  = 0x00040000,
  MASSES_MASK      = 0x00080000
};
// clang-format on
...
template <class DeviceType> class MLIAPDataKokkos : public MLIAPData {
 public:
  ...
  DAT::tdual_float_2d k_positions;            // position vector of each atom
  DAT::tdual_float_2d k_velocities;            // velocity vector of each atom
  DAT::tdual_float_1d k_masses;            // mass of each atom
...
public:
  ...
  MLIAPDataKokkosDevice(MLIAPDataKokkos<LMPDeviceType> &base) :
    ...
    egradient(nullptr),
    positions(base.k_positions.d_view.data()),  // SW new
    velocities(base.k_velocities.d_view.data()),  // SW new
    masses(base.k_masses.d_view.data()),  // SW new
    ntotal(base.ntotal),
  ...
  // SW new data to be passed to the python object
  double *positions;
  double *velocities;
  double *masses;
