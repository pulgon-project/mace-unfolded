...
#include "kokkos.h"
#include "memory_kokkos.h"   // added, would be nicer to make the new implementation consistent with the original
...
MLIAPDataKokkos<DeviceType>::~MLIAPDataKokkos() {
  ...
  // new destroy calls
  memoryKK->destroy_kokkos(k_positions,positions);
  memoryKK->destroy_kokkos(k_velocities,velocities);
  memoryKK->destroy_kokkos(k_masses,masses);
}
...
void MLIAPDataKokkos<DeviceType>::generate_neighdata(class NeighList *list_in, int eflag_in, int vflag_in) {
  ...
  if (atom->nmax > nmax) {
    ...
    memoryKK->destroy_kokkos(k_positions,positions);
    memoryKK->create_kokkos(k_positions, positions, nmax, 3, "mliap_data:positions");
    memoryKK->destroy_kokkos(k_velocities,velocities);
    memoryKK->create_kokkos(k_velocities, velocities, nmax, 3, "mliap_data:velocities");
    memoryKK->destroy_kokkos(k_masses,masses);
    memoryKK->create_kokkos(k_masses, masses, nmax, "mliap_data:masses");
  ...
  // SW added lines
  auto d_positions = k_positions.template view<DeviceType>();
  auto d_velocities = k_velocities.template view<DeviceType>();
  auto d_masses = k_masses.template view<DeviceType>();
  ...
  Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType>(0,nmax), KOKKOS_LAMBDA (int i)  {
    ...
    d_velocities(i, 0) = v(i, 0);
    d_velocities(i, 1) = v(i, 1);
    d_velocities(i, 2) = v(i, 2);
    d_masses(i) = mass[itype];
  });

  modified(execution_space, NUMNEIGHS_MASK | IATOMS_MASK | IELEMS_MASK | ELEMS_MASK | JATOMS_MASK | PAIR_I_MASK | JELEMS_MASK | RIJ_MASK | IJ_MASK | POSITIONS_MASK | VELOCITIES_MASK | MASSES_MASK );
  eflag = eflag_in;
  vflag = vflag_in;
}
...
void MLIAPDataKokkos<DeviceType>::modified(ExecutionSpace space, unsigned int mask, bool ignore_auto_sync) {
  if (space == Device) {
    ...
    if (mask & POSITIONS_MASK   ) k_positions      .modify<LMPDeviceType>(); // SW - added
    if (mask & VELOCITIES_MASK  ) k_velocities     .modify<LMPDeviceType>();
    if (mask & MASSES_MASK      ) k_masses         .modify<LMPDeviceType>();
    ...
    if (mask & POSITIONS_MASK   ) k_positions      .modify<LMPHostType>(); // SW - added
    if (mask & VELOCITIES_MASK  ) k_velocities     .modify<LMPHostType>();
    if (mask & MASSES_MASK      ) k_masses         .modify<LMPHostType>();
...
void MLIAPDataKokkos<DeviceType>::sync(ExecutionSpace space, unsigned int mask, bool ignore_auto_sync) {
  ...
  if (space == Device) {
    ...
    if (mask & POSITIONS_MASK   ) k_positions      .sync<LMPDeviceType>(); // SW - added
    if (mask & VELOCITIES_MASK  ) k_velocities     .sync<LMPDeviceType>();
    if (mask & MASSES_MASK      ) k_masses         .sync<LMPDeviceType>();
    ...
    if (mask & POSITIONS_MASK   ) k_positions      .sync<LMPHostType>(); // SW - added
    if (mask & VELOCITIES_MASK  ) k_velocities     .sync<LMPHostType>();
    if (mask & MASSES_MASK      ) k_masses         .sync<LMPHostType>();
  }
}
