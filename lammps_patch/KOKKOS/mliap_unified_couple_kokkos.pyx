...
cdef extern from "mliap_data_kokkos.h" namespace "LAMMPS_NS":
    cdef cppclass MLIAPDataKokkosDevice:
        ...
        # SW new data to be passed to python
        double * positions
        double * velocities
        double * masses
...
cdef class MLIAPDataPy:
    ...
    @property
    def positions(self):
        if self.data.positions is NULL:
            return None
        return create_array(self.data.dev, self.data.positions, [self.ntotal,3],False)

    @property
    def velocities(self):
        if self.data.velocities is NULL:
            return None
        return create_array(self.data.dev, self.data.velocities, [self.ntotal,3],False)

    @property
    def masses(self):
        if self.data.masses is NULL:
            return None
        return create_array(self.data.dev, self.data.masses, [self.ntotal],False)
