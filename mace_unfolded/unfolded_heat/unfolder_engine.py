"""
Some of the files present in this directory are modified versions of scripts taken from the associated publication:
Marcel F. Langer, Florian Knoop, Christian Carbogno, Matthias Scheffler, and Matthias Rupp
Phys. Rev. B 108, L100302

Please credit the original authors accordingly!
"""
...
def get_unfolding(positions, cell, cutoff, pbc=[True, True, True]):
    ...
    if pbc[0]:
        ...
        collisions[:, 0] = np.zeros_like(projections_x)
        collisions[:, 1] = np.zeros_like(projections_x)

    if pbc[1]:
        collisions[:, 2] = projections_y <= cutoff
        collisions[:, 3] = projections_y >= cell.heights[1] - cutoff
    else:
        collisions[:, 2] = np.zeros_like(projections_y)
        collisions[:, 3] = np.zeros_like(projections_y)

    if pbc[2]:
        collisions[:, 4] = projections_z <= cutoff
        collisions[:, 5] = projections_z >= cell.heights[2] - cutoff
    else:
        collisions[:, 4] = np.zeros_like(projections_z)
        collisions[:, 5] = np.zeros_like(projections_z)
